...
def run_one_sim(idx, params: Parameters, days, generation, algorithm, seed = ga_config.RANDOM_SEED[0],):
    set_seed(seed)
    t0 = time.time()
    ...
    sim = Simulation()
    sim.create_scenario()
    weekly_csv = sim.run_simulation(days=days, GPU_idx=idx, generation=generation, algorithm=algorithm)
